...
fixUser() {
  case "$2" in
    U) if query "$1" "not needed user" "lock account"; then
        usermod -L -e 1 "$1"
       else
        :
       fi;;
    R) if query "$1" "UID=0" "set UID to GID"; then
         ...
         sed -i "s/:0:$gid:/:$gid:$gid:/" /etc/passwd
       else
        :
       fi;;
    ...
    S) if query "$1" "in sudoers file" "remove user from /etc/sudoers"; then
        for n in $(grep -n -v "#" /etc/sudoers | grep "$1" | awk -F: '{print $1}'); do
          sed -i "${n}d" /etc/sudoers
        done
       else
        :
       fi;;
    ...
    K) if query "$1" "authorized_keys SSH" "disable the keys"; then
        mv "/home/$1/.ssh/authorized_keys" "/home/$1/.ssh/unauthor1zed_keys"
       else
        :
       fi;;
  esac
...
size=$(grep -n -e Y -e N userList.txt | tail -1 | awk -F: '{print $1}')
line=1
